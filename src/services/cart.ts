...
export default class CartService {
  ...
  static findByProductId(productId: string) {
    return new Promise((resolve, reject) => db.transaction(tx => {
        tx.executeSql(`select * from ${table} where product_id=?`, [productId], (_, { rows }) => {
            resolve(rows)
        }), (sqlError: any) => {
  ...
  static updateByProductId(cart: CartModel) {
    return new Promise((resolve, reject) => db.transaction(tx => {
        tx.executeSql(`update ${table} set quantity = ? where product_id = ?;`, [cart.quantity, cart.product_id], () => {
      }), (sqlError: any) => {
        console.log(sqlError);
      }}, (txError) => {
        console.log(txError);
  ...
  static deleteByProductId(productId: string) {
    db.transaction(
      tx => {
          tx.executeSql(`delete from ${table} where product_id = ?;`, [productId], (_, { rows }) => {
          }), (sqlError: any) => {
              console.log(sqlError);
          }}, (txError) => {
          console.log(txError);
